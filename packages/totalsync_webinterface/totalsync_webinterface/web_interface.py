...
# web/ lives inside this package rather than beside it, so the build backend ships it
# without any package-data declaration to keep in step -- get that wrong and the wheel
# installs an interface with no JS. importlib.resources then finds it wherever the
# package ends up, with no '..' arithmetic that a move would silently invalidate.
#
# os.fspath() because SimpleHTTPRequestHandler(directory=) and read_text() both want a
# real filesystem path. For a normally installed wheel the traversable *is* one; if the
# package were ever zipimported this raises instead of quietly serving nothing.
WEB_DIRECTORY = os.fspath(resources.files(__package__) / 'web')
